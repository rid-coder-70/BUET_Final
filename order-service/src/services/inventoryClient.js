...
const CIRCUIT_BREAKER_OPTIONS = {
  timeout: parseInt(process.env.CIRCUIT_BREAKER_TIMEOUT) || 30000, // 30s timeout for circuit breaker
  errorThresholdPercentage: parseInt(process.env.CIRCUIT_BREAKER_ERROR_THRESHOLD) || 50, // Open circuit at 50% errors
  resetTimeout: parseInt(process.env.CIRCUIT_BREAKER_RESET_TIMEOUT) || 10000 // Try again after 10s
};
...
/**
 * Update inventory - wrapped function for circuit breaker
 */
const updateInventoryFunction = async (productId, quantity, orderId, idempotencyKey) => {
  ...
  try {
    ...
    if (error.code === 'ECONNABORTED') {
      console.error(`⏱️  Request timeout after ${elapsed}ms (configured: ${REQUEST_TIMEOUT_MS}ms)`);
...
/**
 * Create Circuit Breaker for inventory updates
 */
...
inventoryCircuitBreaker.on('timeout', () => {
  console.warn('⏱️  CIRCUIT BREAKER TIMEOUT: Request took too long');
});
...
/**
 * Update inventory with timeout, retry, and circuit breaker protection
 * 
 * @param {string} productId - Product identifier
 * @param {number} quantity - Quantity to deduct
 * @param {string} orderId - Associated order ID
 * @param {string} idempotencyKey - Idempotency key
 * @returns {Promise} Response from inventory service
 */
